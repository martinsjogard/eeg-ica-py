...
def msfun_meg_ica_cumulantanalysis(IC, cfg):
    if 'S' not in IC or not isinstance(IC['S'], np.ndarray) or IC['S'].ndim not in [2, 3]:
        raise ValueError('msfun_meg_ica_cumulantanalysis - ERROR : IC structure missing elements or inconsistent.')
    ...
    if not isinstance(cfg, dict):
        raise ValueError('msfun_meg_ica_cumulantanalysis - Configuration must be a structure... Try again.')

    Tskew = cfg.get('Tskew', np.nan)
    Tkurt = cfg.get('Tkurt', 15)

    if epoching:
        print('msfun_meg_ica_cumulantanalysis - Baseline correcting and concatenating epochs...')
    ...
    print('msfun_meg_ica_cumulantanalysis - Computing IC skewness and kurtosis...')
    skew_vals = skew(IC['S'], axis=1, bias=False)
    kurt_vals = kurtosis(IC['S'], axis=1, bias=False)  # excess kurtosis by default is True

    print('msfun_meg_ica_cumulantanalysis - Classifying ICs using their kurtosis...')
    ...
    print('msfun_meg_ica_cumulantanalysis - Done.')
    return IC